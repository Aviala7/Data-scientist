...
#  return i
#end
#puts "Il y a #{five_letters(array)} handles avec 5 lettres"

#Combien commencent par une majuscule (première lettre juste après le @) ?
#upper_case=$list.grep(/@[A-Z]/).count
#puts " Il y a #{upper_case} handle avec des majuscules aprés le 0"
#puts capital_count "Il y a #{i} handles avec une majuscule"

#Trie la liste de handle par ordre alphabétique.
#handle_alphabetic = array.sort_by(&:downcase)
#puts handle_alphabetic
...
#position_epenser = array.index ("@epenser")
#puts "Epenser est à la #{position_epenser} place dans la liste."
...
array.each do |tweet|
  if !sortedTweets.key?(tweet.length)
    sortedTweets[tweet.length] = 1
  else
    sortedTweets[tweet.length] += 1
  end
end
...
sortedTweets.each do |length, count|
  puts "Il y a #{count} #{ count > 1 ? 's' : ''}handles avec #{length} lettre#{length > 1 ? 's' : ''}"
end
